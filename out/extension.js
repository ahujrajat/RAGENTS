...
const ragentsProvider_1 = require("./ragentsProvider");
...
function activate(context) {
    ...
    const ragentsProvider = new ragentsProvider_1.RagentsProvider(rootPath);
    ...
    vscode.commands.registerCommand('ragents.preview', async (filePath, isPublic) => {
    ...
    vscode.commands.registerCommand('ragents.download', async (item) => {
        ...
        const platformId = await (0, platformConfig_1.resolveTargetPlatform)();
        if (!platformId) {
            return;
        }
        const platform = platformConfig_1.PLATFORMS[platformId];
        ...
        try {
            ...
            if (stat.isDirectory()) {
                if (platformId === 'claudeCode') {
                    ...
                    if (fs.existsSync(skillMdPath)) {
                        ...
                        if (fs.existsSync(flatDestPath)) {
                            const overwrite = await vscode.window.showWarningMessage(`"${flatFileName}" already exists in ${displayPath}. Overwrite?`, 'Overwrite', 'Cancel');
                            if (overwrite !== 'Overwrite') {
                                return;
                            }
                        }
                        fs.writeFileSync(flatDestPath, content);
                        vscode.window.showInformationMessage(`Downloaded skill "${originalFileName}" to ${displayPath} as slash command.`);
                    ...
                    if (category === 'skills') {
                        const action = await vscode.window.showInformationMessage(`Downloaded skill "${originalFileName}" to ${displayPath}. Run /skills reload in chat to make it available.`, 'Copy Command');
                ...
                if (platformId === 'claudeCode' && (category === 'prompts' || category === 'skills') && fs.existsSync(destPath)) {
                    const overwrite = await vscode.window.showWarningMessage(`"${fileName}" already exists in ${displayPath}. Overwrite?`, 'Overwrite', 'Cancel');
                    if (overwrite !== 'Overwrite') {
                        return;
                    }
                }
...
function bundleSkillDirectory(dirPath) {
